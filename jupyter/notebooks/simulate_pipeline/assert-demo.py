...
def create_sas(table):
    blob = get_blob_sas(account_name,account_key, container_name, table)
    sas_url = 'https://'+account_name+'.blob.core.windows.net/'+container_name+'/'+table+'?'+blob
    return sas_url


def get_data(table):
    sas_url = create_sas(table)
...
price, savings = assert_log_analytics_logic(tall)
